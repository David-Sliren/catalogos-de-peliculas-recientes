...
function Btn({ text, onActive, isLeft }) {
  return (
    ...
    >
      {!isLeft && (
        <p className="text-lg text-zinc-900 font-inter font-semibold group-hover:text-white">
          {text}
        </p>
      )}
      <svg
        className={`w-8 h-8 justify-end group-hover:bg-gray-50 text-gray-50 ease-linear duration-300 rounded-full border border-gray-700 group-hover:border-none p-2 ${
          isLeft ? "group-hover:-rotate-90" : "group-hover:rotate-90"
